...
```text
               ...
               /   |   \          /   |   \      /  |  \
         score(2) sc(1) sc(0)  sc(1) sc(0) -1   0  -1  -2
          / | \    | | \  |     | | \  |
       sc(1) ...  ...    ...   ...
```
...
```text
1. total_recur([30, 40, 50])
   => Extract 30, recurse on [40, 50]

2. 30 + total_recur([40, 50])
        => Extract 40, recurse on [50]

3. 30 + 40 + total_recur([50])
             => Extract 50, recurse on []

4. 30 + 40 + 50 + total_recur([])
                  => Base case [] returns 0

5. Pass the sum back up the chain:
   30 + 40 + 50 + 0 = 120
```
