...
| **Base Case** | The stopping condition that returns a value without making any subsequent recursive calls. |
| **Recursive Step** | The part of the function where the problem is reduced and a recursive call is made. |
| **Call Stack** | The internal mechanism Python uses to keep track of active subroutines. Deep recursion can lead to a `RecursionError`. |
| **Tree Recursion** | When a function makes multiple recursive calls to itself (e.g., Naive Fibonacci). Often leads to exponential time complexity `O(2^n)`. |
| **Memoization** | Caching the results of expensive function calls to avoid redundant work, typically reducing `O(2^n)` to `O(n)`. |
| **State Management** | State should either be passed explicitly as arguments (down) or aggregated through return values (up) instead of relying on globals. |
