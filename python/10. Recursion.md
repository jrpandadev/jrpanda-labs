...
## 🏛️ MIT Challenge: Recursion and Caesar Cipher (Problem Set 4)

### Introduction
This problem set is split into two topics: the first focuses on recursion and trees (Part A), while the second looks at classes and cryptography (Parts B and C).
...
We will be using a provided `Node` object in `tree.py` to represent trees.
- Initialize with `Node(value, left_child, right_child)`.
- Get children with `get_left_child()` or `get_right_child()`.
- Get value with `get_value()`.

#### 1.2) Determining The Height of a Tree
The height of a tree is the number of edges between the root and the furthest leaf. Write a recursive function, `find_tree_height`, that determines the depth of a tree. This function must be recursive.

**Hint:**
- **Base case:** If it is a leaf, it has a height of 0.
- **Recursive case:** Recursively find the height of its left and right subtrees and take the maximum of the two. Add 1 to the maximum height and return that value.

#### 1.3) Heaps
A special type of trees are heaps (max heaps and min heaps).
Write the function `is_heap` to quickly determine if a tree is a max or min heap, depending on the `compare_func` parameter. The `compare_func` returns `True` if `child_value < parent_value` for max heaps, and `True` if `child_value > parent_value` for min heaps.

**Hint:**
- **Base case:** If it is a leaf then it is a heap.
- **Recursive case:** Check if right and left subtrees are heaps and that the root is the max or min element of its subtree (using `compare_func`).

---

### 2) Part B: Encryption with One Time Pads

#### 2.1) How a One Time Pad Works
The idea of a one time pad is to "shift" each character in your plaintext message by a random amount. We are interested in ASCII characters from 32 (space) to 126 (~). We need to properly wrap around this range (length 95) if a shift pushes a character outside these bounds.

#### 2.2) Message Class
Create a parent class `Message`.
- `__init__(self, input_text)`
- `get_text(self)`
- `shift_char(self, char, shift)`: Shift character and wrap around 32 to 126.
- `apply_pad(self, pad)`: Apply the shift list to the text.

#### 2.3) PlaintextMessage Class (Inherits from Message)
- `__init__(self, input_text, pad=None)`
- `generate_pad(self)`: List of random ints `[0, 110)` equal to text length.
- `get_pad(self)`: Return a copy of pad.
- `get_ciphertext(self)`: Apply pad to get ciphertext.
- `change_pad(self, new_pad)`

#### 2.4) EncryptedMessage Class (Inherits from Message)
- `__init__(self, input_text)`
- `decrypt_message(self, pad)`: Return decrypted string.

---

### 3) Part C: Using Your Classes
Find the correct pad by trying all pads and returning the one that produces the most valid English words.
- `decrypt_message_try_pads(self, pads)`
- `decode_story()`


<details>
<summary><b>💡 Click to Reveal Complete PS4 Solution (Parts A, B & C)</b></summary>
...
def is_heap(tree, compare_func):
    """
    Recursively determines if a tree is a max/min heap.
    """
    if tree.get_left_child() is None and tree.get_right_child() is None:
        return True
    ...
    if tree.get_left_child() is not None:
        if not compare_func(tree.get_left_child().get_value(), tree.get_value()):
            return False
        left_heap = is_heap(tree.get_left_child(), compare_func)
        
    if tree.get_right_child() is not None:
        if not compare_func(tree.get_right_child().get_value(), tree.get_value()):
            return False
        right_heap = is_heap(tree.get_right_child(), compare_func)
        
    return left_heap and right_heap

# ==========================================
# PART B: ENCRYPTION WITH ONE TIME PADS
# ==========================================
...
class Message(object):
    ...
    def shift_char(self, char, shift):
        ...
        if 32 <= ascii_val <= 126:
            ...
            # Wrap around logic (range length is 95)
            while shifted > 126:
                shifted -= 95
            while shifted < 32:
                shifted += 95
    ...
    def apply_pad(self, pad):
        ...
        for i, char in enumerate(self.message_text):
            ciphertext += self.shift_char(char, pad[i])
        return ciphertext
...
class EncryptedMessage(Message):
    ...
    def decrypt_message(self, pad):
        # Reverse the shift by negating the pad values
        decryption_pad = [-shift for shift in pad]
        return self.apply_pad(decryption_pad)

# ==========================================
# PART C: USING YOUR CLASSES
# ==========================================
# (Assumes helper functions `is_word`, `get_story_string`, `get_story_pads` exist)

class EncryptedMessageWithDecryption(EncryptedMessage):
    def decrypt_message_try_pads(self, pads, wordlist):
        max_words = 0
        best_plaintext = ""
        
        for pad in pads:
            decrypted = self.decrypt_message(pad)
            words = decrypted.split(' ')
            # Count how many are valid English words
            valid_word_count = sum(1 for w in words if is_word(wordlist, w))
            
            # >= handles returning the LAST pad in the event of ties
...
def decode_story():
    ...
    pads = get_story_pads()
    # wordlist = load_words(...) # Handled by helpers
    encrypted = EncryptedMessageWithDecryption(ciphertext)
    return encrypted.decrypt_message_try_pads(pads, wordlist)
